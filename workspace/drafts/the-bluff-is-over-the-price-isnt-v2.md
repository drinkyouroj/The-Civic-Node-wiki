...
The memory market currently has everything a bear case needs. Google released a compression breakthrough that [compresses AI inference memory 6x](https://research.google/blog/turboquant-redefining-ai-efficiency-with-extreme-compression/). Half of America's planned data centers have been cancelled or delayed. Chinese factories are ramping toward volume production. By every standard model of supply and demand, DDR5 should be trending toward $200 by now.
...
- Inline source links: 18 (up from 14 in v1)
- Marcus tests: all pass (unchanged from v1)

**Reconciliation (v1 → v2):**
- Corrections applied: 11

- **Figures corrected:**
  - Opener: "reduces AI memory requirements by 80%" → "compresses AI inference memory 6x" (source says 6x, not 80%; internal consistency with Force 1 restored)
  - Force 1: "March 25" → "March 23" (primary source: Google Research Blog raw frontmatter `published: 2026-03-23`)
  - Force 1: "compresses the memory footprint of large language models by 6x" → "compresses the KV cache — the working memory of AI inference — by 6x" (source specifies KV cache, not total LLM memory footprint)
  - Force 3: "directly affect roughly 50% of Samsung's advanced chip production" → "halt roughly 50% of production at Samsung's Pyeongtaek campus" (source specifies Pyeongtaek campus, not Samsung's global output; source notes figure is from the union chief)

- **Links added (4 unsourced claims now sourced):**
  - Force 1: "Samsung and SK Hynix stocks dropped 5–6%" → linked to Times of India (TOI) article documenting the TurboQuant stock reaction (Tier 3; best available; wiki page: `How Sam Altman's OpenAI may have caused the worst consumer hardware crisis.md`)
  - Force 3: "7% base wage increase and profit-sharing" → linked to Reuters Samsung strike article
  - Force 3: "roughly 50% of production at Samsung's Pyeongtaek campus" → linked to Reuters Samsung strike article
  - Force 2: "Samsung and SK Hynix together produce roughly 70% of global DRAM" → linked to Forbes/Tirias helium article

- **Links swapped (2):**
  - Force 1: WCCFTech 403 URL → TrendForce DDR5 pullback URL for the 30%+ retail drop claim (TrendForce is Tier 2 vs WCCFTech Tier 3; same underlying data; TrendForce also specifically covers Huaqiangbei)
  - Force 2: DigiTimes (sponsored content) → Forbes/Tirias for the "six months of forward contracted supply" claim (Forbes independently confirms the same figure; Tier 2 vs Tier 3)

- **Context added (1):**
  - Force 4: "from China" → "sourced from overseas — including China, South Korea, and Canada" (source lists multiple countries; original phrasing overstated China's exclusivity)

- **Links retained as-is:**
  - Bloomberg Stargate link: kept despite paywall (Bloomberg is Tier 2; paywall links are acceptable in Substack; Futurism Tier 3 alternative was deprioritized)

- **Skipped / unresolved:**
  - "$40-per-hour / $7" inference cost: classified as editorial illustration, no source required (confirmed in fact-check)
  - "near-zero accuracy loss" (Force 1): classified as acceptable conservative hedge vs. source's "zero"; not changed per reconcile-only-what-was-flagged rule

- **Wiki audit:** 18 source links in v2
  - Already ingested (wiki/sources/): 17 ✓
  - In raw/ only: 0
  - New raw files created: 0
  - All URLs tracked in wiki — no gaps
