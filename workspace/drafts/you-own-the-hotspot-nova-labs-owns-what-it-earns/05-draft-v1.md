...
Aggregate disclosure on data-transfer revenue, audited annually, even when individual carrier rate sheets stay confidential. The unit economics can stay private. The total revenue split can't.
...
Sunset provisions that require an active re-vote before the authority extends. HIP-143's 1-year sunset was the right design. The execution gap — nothing requiring a new vote before the authority extended — is fixable.
...
The four-part standard would not have caught Datagram. A project can publish exit provisions, revenue disclosure, geographic acknowledgment, and re-vote sunsets, and still have no business underneath any of it. The structural problem — governance that lets the proposing entity accumulate authority over time — and the fraud problem share an architecture. Both require an operator who deploys capital before the documents governing what they bought are written.

I deployed twelve nodes without reading the documents. That's the debug.
...
- Marcus tests: All pass. Signal test strongest — primary-source vote breakdowns + ferebee +55.5% trajectory finding not available in secondary coverage. Patience test: Section 2 ¶1 (charitable reading) is kept under 70 words to prevent Marcus attrition at the densest entry point.
- Inline source links: 18
- Unsourced claims: none. All source links verified. The proxy-delegation quote ("set a proxy as a backup to ensure you don't miss out on rewards") confirmed at line 83 of raw/Helium Halving 2025 What It Means for Hotspot Operators, HNT Holders, and Network Governance.md — same URL as the halving blog link in the draft. ✓
- Voice flags: (1) "The audit is the floor. It doesn't catch fraud." — FIXED from outline draft. (2) Closing line ends on "before the hardware ships" — "not after" dropped. FIXED. (3) Section 2: "Present them fairly" as a self-directed imperative sentence; intended as deadpan signal that the audit is about to start. Flag for user judgment.
- Opener-close contract honored: Close echoes the payback math ("does the math before the hardware ships, not after") from the opener's "ten years. Twenty." Callback is clean.
