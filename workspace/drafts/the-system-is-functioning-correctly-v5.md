...
There's a name for this: **institutional gaslighting** — systematic institutional lying that floods the zone with competing claims until finding the truth becomes too expensive for most people to bother. The goal isn't persuasion. It's exhaustion. Make accountability so complicated and politically toxic that the public gives up and accepts the institution's version by default.
...
When institutions make truth too expensive to find, [conspiracy theories fill the gap](https://www.npr.org/2026/01/02/nx-s1-5662638/epstein-files-release-trump-conspiracy-2026). The Epstein release is the cleanest example: the redactions fuel speculation; the multi-year timeline guarantees the truth arrives too late to matter. Institutional gaslighting doesn't just prevent accountability; it poisons the information environment until even honest investigators can't separate signal from noise.

Cigna's PxDx denied 300,000 claims in two months at 1.2 seconds per decision. That is a currently operating system. The same prior-approval system now runs across every major individual market insurer — the plans that cover every freelancer, solo operator, and anyone who left employer-sponsored coverage to work for themselves. The exhaustion engine no longer needs humans to operate the delay machinery. It runs at 1.2 seconds per decision. Your capacity to appeal does not scale at that rate.
...
**v4.1 → v5 changes (reading level normalization, target: grade 10):**
- "determining truth becomes too costly for most people to pursue" → "finding the truth becomes too expensive for most people to bother" (abstract gerund → concrete verb)
- "procedurally complex" → "complicated" (grade 16 → grade 8 vocabulary)
- "whoever controls the evidence of their own conduct" → "whoever controls the evidence of their own wrongdoing" (more concrete)
- "replace the investigation with an investigation-shaped object" → "replace the investigation with something that looks like one" (removes abstraction while keeping the irony)
- "document productions that produce no consequence" → "document dumps that produce no consequences"
- "the statute of limitations gives up" → "the legal deadline gives up" (unexplained legal term → plain language; two subsequent uses also converted)
- "toothless legal instruments: laws designed to produce accountability that omit the enforcement mechanisms that would produce it" → "toothless laws: rules written to require accountability that include no way to enforce them"
- "The architecture is complete when the law itself contains no teeth" → "The architecture is complete when the law itself has no teeth" (minor: "contains no teeth" is awkward idiom)
- "evidence suppression" → "locking out investigators" (legal term → concrete description)
- "unanimous consent" → "unanimous agreement"
- "6 million potentially responsive pages" → "6 million pages that could be relevant"
- "no penalty for noncompliance" → "no penalty for ignoring it"
- "there was no consequence available" → "nothing happened"
- "Inspector General" → "government's internal watchdog" (bureaucratic title → function-first description)
- "Nothing attached." → "Nothing attached to make it enforceable." (adds the implied meaning for readers unfamiliar with how congressional oversight requests work)
- "articulating that institution size overrides individual accountability" → "saying that bank size overrides individual accountability" (simpler verb; "institution" already established in context)
- "overly cautious reading of pre-election policy" → "concerns about prosecuting during election season" (jargon → plain statement of the actual concern)
- "five-year statute of limitations expires" → "five-year legal deadline for charges expires"
- "the statute of limitations had expired for nearly everyone else" → "the legal deadline for charges had passed for nearly everyone else"
- "procedural control over its own accountability" → "enough control over its own accountability process"
- "good-faith investigators" → "honest investigators"
- "prior-authorization logic" → "prior-approval system"
