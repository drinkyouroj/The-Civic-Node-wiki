...
an animated face into reserved negative space in post-production — so the
deck must LEAVE that space empty (see "Reserved face-space layout" below
and the per-scene notes). Do not fill the reserved region with text or
decoration.
...
phone). **The primary recording aspect is 1:1 (1080×1080).** The 16:9 and
9:16 outputs are derived from the same HTML by changing the recording
window's aspect — slide content does not reflow.
...
- Each `.slide` element uses `container-type: size` so cq-units scale to
  the slide, not the viewport.
...
- Critical content (kicker, headline, body, hero numbers, source
  attributions, CTA, disclosure) lives ONLY inside the safe zone. The
  reserved face-space column stays empty (the face is composited in post).
- Decorative elements (brand mark, `sl-hairline`, `sl-glow` radial, slide
  background fill) may extend to slide edges.
- No fixed-pixel widths on layout containers. No aspect-ratio media
  queries. Layout is identical at every aspect; only the empty margin
  differs.
...
No element renders below `--type-kicker`. Use `--type-h2-mid` on the Twist
slide (Slide 07) — its text lives in the 42% left column with the right
side reserved for the face composite, so `--type-h1` would overflow the
narrow column.
...
`slide-stakes`, `slide-twist`, `slide-tease`, `slide-end`. Example:
`<section class="slide slide-twist" id="slide-07">`.

**Reserved face-space layout** (no character image is rendered — the
presenter composites an animated face into the empty side column in post).
Keep each figure slide's content in ONE side column and leave the opposite
column as clear negative space. Same proven geometry as the 2026-05-25
hand-test, minus the image:

```css
/* Receipt + Stakes slides — content RIGHT 65%; LEFT ~32% reserved EMPTY for the face */
.slide-receipt .stat-block, .slide-stakes .stat-block {
  position: absolute;
  right: 0;
  top: calc(var(--type-kicker) + 2cqh);
  bottom: calc(var(--type-kicker) + 2cqh);
  width: 65%;
  display: flex; flex-direction: column; justify-content: center;
}
.slide-receipt .hero-num, .slide-stakes .hero-num {
  font-size: var(--type-hero); line-height: 0.9; color: #fff;
  font-weight: 700; letter-spacing: -0.02em;
}
.slide-receipt .stat-label, .slide-stakes .stat-label {
  font-size: var(--type-body); color: var(--paper);
  margin-top: 0.4em; max-width: 90%; line-height: 1.15; text-wrap: balance;
}
/* The left column (left:0 → ~32%) stays EMPTY. No text, no decoration there. */

/* Twist slide — text LEFT 42%; RIGHT ~55% reserved EMPTY for the face */
.slide-twist .text-col {
  position: absolute;
  left: 0;
  top: calc(var(--type-kicker) + 2cqh);
  bottom: calc(var(--type-kicker) + 2cqh);
  width: 42%;
  display: flex; flex-direction: column; justify-content: center; gap: 1.5cqh;
}
.slide-twist .headline {
  font-size: var(--type-h2-mid); line-height: 1.05; color: #fff;
  text-wrap: balance;
}
.slide-twist .caption {
...
/* The right column (right:0 → ~55%) stays EMPTY for the face composite. */
```

Keep the reserved column genuinely empty at every aspect — the safe-zone is
square, so the reserved fraction holds at 1:1 / 16:9 / 9:16. Decorative
hairlines / glow may cross it, but no legible content goes there. The face
is added in post; at build time the reserved column is blank.

**Thumbnail-anchor rule:** every scene has exactly one element at
`--type-hero` (or `--type-h1`, or `--type-h2-mid` on Slide 07)
occupying ≥20% of safe-zone height. This is the element the deck rests on
at each scene's final beat — what a 240px phone card catches. Per-scene
anchor named below.
...
named below keep each resting frame inside budget. Speaker notes and
the reserved face-space column are not counted.
...
### Scene 01 — `sl-title` · HOOK · 10 beats · no Justoon

**Kicker:** `DISPATCH №006 · HOOK`
**Thumbnail anchor (resting state):** `100 : 1` at `--type-hero`, with
`$400,000` and `$4,000` at `--type-h1` flanking the two figures.
...
- **B10** "A hundred to one." → **[RESET to anchors]** dim the labels + stamps to faint; `100 : 1` lands center at `--type-hero` with `sl-glow` radial slate behind it; `$400,000` / `$4,000` stay as the flanking comparison. **Hold ~1.5s.**
...
### Scene 02 — `sl-lead` · THESIS · 9 beats · no Justoon
...
### Scene 03 — `sl-data` (ms-numgrid) · THE RECEIPT · THE CHOKEPOINT · 13 beats · reserve left ~32%

**Kicker:** `DISPATCH №006 · THE RECEIPT · THE CHOKEPOINT`
**Reserved face-space:** keep the **left ~32%** of the safe zone empty (per
`.slide-receipt .stat-block` layout) — the presenter composites the
animated face there. Stats build in the right 65%.
**Thumbnail anchor:** `10.5%` at `--type-hero` with `sl-glow` (resting state).

Group 1 — the irreplaceable input **[RESET after B4]**:
- **B1** "So why did the chip worker win?" → the right-side stat-block frame opens (left column stays clear for the face)
...
- **B12** "Ten and a half percent of the division's profit." → `10.5%` hero number lands (`--type-hero`), `sl-glow` behind it
- **B13** "Every year. For ten years." → `EVERY YEAR ×10` (ten ticks fill a row beneath the hero). **Hold ~1s.** Resting frame: `10.5%` hero + `+755%` supporting + `EVERY YEAR ×10` (all in the right 65%; left column blank for the face).
...
### Scene 04 — `sl-frames` · THE FRAME · WHO CAN SAY NO · 12 beats · no Justoon
...
### Scene 05 — `sl-data` · THE RECEIPT · THE OTHER CHOKEPOINT · 13 beats · reserve left ~32%

**Kicker:** `DISPATCH №006 · THE RECEIPT · THE OTHER CHOKEPOINT`
**Reserved face-space:** left ~32% of the safe zone empty (face composite);
content builds in the right 65%.
...
- **B1** "Now follow the electricity instead of the chips." → a power line traces across the right column, away from the chips (left stays clear for the face)
...
### Scene 06 — `sl-data` · THE RECEIPT · THE BILL · 10 beats · reserve left ~32%

**Kicker:** `DISPATCH №006 · THE RECEIPT · THE BILL`
**Reserved face-space:** left ~32% of the safe zone empty (face composite);
content, including the `$13.77 BILLION` hero, builds in the right 65%.
**Thumbnail anchor:** `$13.77 BILLION` at `--type-hero` with `sl-glow`
...
- **B1** "The difference is that nobody here has a hand on the valve." → a valve with no hand on it (right column; left stays clear for the face)
...
- **B4** "Data-center demand added almost fourteen billion dollars to customer bills." → `$13.77 BILLION` hero counts up (`--type-hero`), `sl-glow` behind it. **Hold a beat.**
...
### Scene 07 — `sl-compare` (before/after) · THE TWIST · IT DOESN'T ROLL DOWNHILL · 11 beats · reserve right ~55%

**Kicker:** `DISPATCH №006 · THE TWIST · IT DOESN'T ROLL DOWNHILL`
**Reserved face-space:** keep the **right ~55%** of the safe zone empty
(per `.slide-twist .text-col` layout) — the presenter composites the
animated face there, ideally a deadpan / flat register to match the dry
correction. Text builds in the left 42%.
**Thumbnail anchor:** `NOT THE SAME MONEY` at `--type-h2-mid` (B11), in the
left text column. No character image renders at build time; the face is
added in post, so this text anchor carries the 240px thumbnail test.
**Layout:** `.text-col` left (42%); right ~55% reserved empty. Beats build
in the text column; the headline of record is `NOT THE SAME MONEY` (B11).

Group 1 — the easy story **[RESET after B5]**:
- **B1** "Here's where the easy version of this story falls apart." → a clean, tidy arrow drawn across the left text column (right stays clear for the face)
...
- **B11** "They are not the same money." → `NOT THE SAME MONEY` lands at `--type-h2-mid` as the headline; two separate money stacks with a gap between them; `sl-glow` behind it. **Longest hold in the deck (~1.5s) — this is the funnel's payoff line.**
...
### Scene 08 — `sl-lead` · THE STAKES · THE PLAYBOOK TRAVELS · 9 beats · reserve left ~32%
...
- **B1** "And this is already spreading." → a ripple spreading across the right column over a faint world map (left stays clear for the face)
...
- Renders correctly at 16:9 (1920×1080), 9:16 (1080×1920), and 1:1
  (1080×1080) from this same file. Test by resizing to each aspect — type
  sizes and safe-zone content stay identical; only the empty margin changes.
...
- Every resting beat passes the visible-text budget (≤25 visible words OR
  one hero number + ≤15 supporting words). The reserved face-space column
  and speaker notes are excluded.
- The reserved negative-space column (left ~32% on Slides 03/05/06/08;
  right ~55% on Slide 07) renders empty — no text, no decoration — at every
  aspect. The presenter composites the animated face there in post.
