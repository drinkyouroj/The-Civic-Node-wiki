...
- Status: review
- Inline source links: 19

**Reconciliation (v2 → v3):**
- Corrections applied: 4
- Claims rewritten:
  - SFist class action: "alleges Flock's contractual prohibition was violated 1.6 million times" → "alleges California's law prohibiting the sharing of driver data with federal and out-of-state agencies was violated 1.6 million times" — the lawsuit's actual legal theory is California state law, not Flock's contract terms (per SFist source as ingested)
- Links added:
  - Flock blog inline on "deployed an immigration keyword filter" (June 2025 claim — Flock blog timeline confirms automatic keyword blocks)
  - Flock blog inline on "added an administrative toggle" (January 2026 claim — Flock blog timeline confirms federal-sharing disable toggle)
- Words changed:
  - CEO quote attribution: "writing that" → "saying" (NPR source attributes as "statement"; medium of written/verbal not specified, so the more general verb is correct)
- Skipped / unresolved:
  - Georgia HB 1105 (2024) — still no source URL available; claim accurate but unsourced [LOW carry-over from v1]
  - Atlanta 2017 City Council resolution language — still no source URL available; claim accurate but unsourced [LOW carry-over from v1]
  - APD activation status on Flock filters — stated as "unknown" throughout; accurate and intentional [N/A]

**Wiki audit — sources linked in v3:** All 8 sources now fully ingested in wiki/sources/ (no raw-only stubs remain).
...
- `https://www.flocksafety.com/blog/does-flock-share-data-with-ice` — ingested ✓ (this v3 pass uses it 4 times; previously stubbed)
