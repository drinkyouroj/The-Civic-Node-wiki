...
**Status:** Draft v2 (review — post-fact-check reconciliation)
...
**Previous version:** `workspace/drafts/bill-of-rights-contractors-door/draft.md`
...
- Inline source links: 13 (up from 10 in v1 — added ATL Press Collective for opener audit, NPR for Langley, Bad Internet Bills for 19 states count, congress.gov for SCREEN Act)
...
- Links swapped: none
- Links fixed: none
- Figures corrected: "4,499 credentialed agencies" → "roughly 4,500 credentialed agencies" (source says "nearly 4,500"; original figure implied more precision than the source supports)
- Context added:
  - §2 ¶2 CISA gloss: "created in 2018 to protect election infrastructure from foreign attacks" → "created in 2018 to protect critical infrastructure from cybersecurity threats" (election infrastructure was one designated subsector, not CISA's founding mission per the House Judiciary report)
  - §2 ¶3 Murthy standing: "plaintiffs couldn't prove a 'concrete link' between government pressure and specific platform actions" → "plaintiffs couldn't show that specific platform actions were traceable to government pressure" (dropped quote marks on phrase not actually in Barrett's opinion; "traceable" is Barrett's actual standing language)
  - §2 ¶3 March 2026 settlement: "It names CISA" → "It names CISA, the Surgeon General, and the CDC" (settlement actually covers three named agencies, not just CISA; analytical point about other-agencies-inheriting-the-instrument unchanged but reinforced)
  - §3 ¶3 echoed the same settlement framing for consistency
  - §2 ¶4 age-verification framing: "Nineteen states have passed online ID-check laws requiring age verification before accessing adult content" expanded slightly to "Nineteen states have passed online ID-check laws requiring residents to hand over a driver's license or a face scan to access age-gated sites, including adult content" (matches the source's broader framing while keeping the adult-content example; reads more concrete)
- Links added:
  - Opener (§1 ¶1): "open-records response" → linked to ATL Press Collective (the audit's primary publication; opener was previously uncited despite carrying the entire frame of the piece)
  - §1 ¶3 Langley: "previously denied" → linked to NPR (Joffe-Block, Feb 2026), the primary source for Langley's denial-to-acknowledgment shift
  - §2 ¶4: "passed online ID-check laws" → linked to Bad Internet Bills (Fight for the Future campaign hub, contains the 19-state count)
  - §2 ¶4: "SCREEN Act" → linked to congress.gov S.737 (primary bill text)
- Claims rewritten:
  - §1 ¶3 Langley denial: was "Flock CEO Garrett Langley had previously said no federal immigration access was occurring" → "Flock CEO Garrett Langley had previously denied the company had federal contracts" (NPR reports the denial was about contracts, not about immigration access specifically; previous phrasing conflated two related but distinct claims) + added "about its relationship with federal agencies" to the acknowledgment to match the actual NPR quote context
- Skipped / unresolved:
  - Beekman stitched quote (§1 ¶2): fact-check flagged as minor; left as-is. Two consecutive utterances from the same speaker, common journalistic practice.
  - Pierce County "one month after" approximation (Aug 25 → Sept 15 = ~21 days): fact-check flagged as acceptable colloquialism; left as-is.
  - "verification vendor holds the biometric database" (§2 ¶4): fact-check flagged as safe analytical assertion; no inline link added, but the new BIB and SCREEN Act links in the same paragraph provide the surrounding context.
  - BetterHelp "exits the HIPAA framework" framing (§2 ¶5): editorial analysis, not a sourced claim. FTC source confirms the $7.8M and the data-sharing practice (both verified); the HIPAA-exit analytical move is the author's. Left as-is.
- Wiki audit: 15 inline source URLs total
  - Already ingested as wiki source pages: 11
    - atlpresscollective.com (Atlanta PD used Flock cameras to track migrants, records show.md)
    - bendsource.com (raw + summary)
    - cbsnews.com Ventura (raw; needs wiki source page)
    - classlawgroup.com (raw; needs wiki source page)
    - sfist.com (raw + wiki source page)
    - judiciary.house.gov CISA report (raw + wiki source page)
    - en.wikipedia.org Murthy v. Missouri (raw + wiki source page)
    - techpolicy.press age verification (raw + wiki source page)
    - ftc.gov BetterHelp (raw + wiki source page)
    - badinternetbills.com (raw + wiki source page)
    - atlantanewsfirst.com (raw + wiki source page)
  - Internal Substack URLs (own published pieces): 2
    - drinkyouroj.substack.com/p/the-jawboning-papers
    - drinkyouroj.substack.com/p/atlanta-passed-a-sanctuary-resolution
  - In raw/ only (needs wiki source page ingestion): 4
    - Bend Source (raw/Bend PD Flock 279 federal queries June 2025.md)
    - Ventura County CBS (raw/Ventura County Flock 364k unauthorized access 2026.md)
    - Gibbs Mura class action page (raw/Gibbs Mura Flock Safety class action California 2026.md)
    - NPR Flock (raw + wiki — confirmed both)
  - New raw files added during reconciliation: 1
    - raw/Text - S.737 - 119th Congress (2025-2026) SCREEN Act.md (full ingestion by user; replaces an interim stub I created during reconciliation)
