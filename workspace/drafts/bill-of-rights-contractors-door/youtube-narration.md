# THE BILL OF RIGHTS ENDS AT THE CONTRACTOR'S DOOR
## The Civic Node · Dispatch №005
## 11 slides · trailer-format · small-screen · 5-7 min target

---

**[SLIDE 01 — HOOK]**

Atlanta passed a sanctuary policy in 2025. Police officers don't help federal immigration enforcement. The city said so on the record.

That same year, the open-records audit showed three thousand two hundred and fifty-four Border Patrol searches of Atlanta's police cameras.

The same audit reported both.

The pattern shows up in four places.

---

**[SLIDE 02 — THESIS]**

Surveillance. Speech. Biometrics. Health data.

In each one, the Constitution writes a rule about what the government can't do. And in each one, the government hires a private company to do the same thing.

The rule applies to the government. The company holds the database.

---

**[SLIDE 03 — RECEIPT · BEND]**

Bend, Oregon. Population ninety-nine thousand.

The police department signed onto Flock, a private camera network used by police agencies. Three weeks later, the captain figured out the federal-search feature was reciprocal. Turn it on to query other cities. Other cities can query yours.

In those three weeks, federal agencies ran two hundred and seventy-nine immigration searches into Bend's cameras. Customs and Border Protection ran one hundred eighteen.

That's the floor. The smallest example.

---

**[SLIDE 04 — RECEIPT · SCALE]**

Now the scale.

Ventura County, California. The sheriff turned the federal-search feature off in 2023, in writing, to comply with state law. Two years later, federal and out-of-state agencies had queried Ventura's cameras three hundred and sixty-four thousand times.

San Francisco. The class action filed in February says outside agencies accessed police cameras one point six million times. In seven months.

There are roughly forty-five hundred police departments on this network.

---

**[SLIDE 05 — FRAME]**

Same architecture every time.

The Constitution writes a rule about the government. So the government finds a private company that isn't bound by the rule. And contracts the work to them.

The rule still exists on paper.

The function still gets performed.

---

**[SLIDE 06 — ANAPHORA · SPEECH + SEARCH]**

Watch it work in two directions.

The First Amendment prohibits the government from ordering speech taken down. So CISA, the federal cybersecurity agency, called the platform. The platform took the post down. Under its own content policy.

The Fourth Amendment requires a warrant for government searches. So the search ran through Flock's private camera network. No warrant needed to query a private company.

The First Amendment prohibits the order. CISA placed the call.

The Fourth Amendment requires the warrant. Flock accepted the query.

---

**[SLIDE 07 — ANAPHORA · BIOMETRIC + HEALTH]**

Two more.

Congress can't compel children to hand over biometric data. So nineteen states passed laws requiring a third-party vendor to collect the face scan. The vendor holds the database. A national security letter reaches it without a judge.

HIPAA protects health records. So a therapy app relabeled its session data as a marketing category. The category isn't HIPAA. The category sold to advertisers.

Congress can't compel the biometric. The verification vendor holds it.

HIPAA protects the record. BetterHelp sold the category.

In each case, the protection was written to govern one party. The work was contracted to another.

---

**[SLIDE 08 — TWIST]**

Back to Ventura.

The sheriff turned the federal-search feature off in 2023. In writing. To comply with state law.

Something turned it back on.

Flock said the cause was impossible to determine. Their own logging system couldn't track which account flipped the setting. The sheriff's own investigation found no one on staff did it.

Constitutional protection. State law. Department policy. All three in alignment.

The setting still got flipped.

By whom, the company says, can't be known.

---

**[SLIDE 09 — STAKES]**

In April 2026, the Atlanta City Council passed two more resolutions.

One opposed ICE detention. The other set new guidelines for Atlanta police.

That makes five. A sanctuary designation. A canceled ICE contract. An open-records audit. Two more resolutions stacked on top.

Neither of the new ones mentions Flock. Neither touches the vendor contract.

The cameras are still on.

The next agency querying them doesn't have to read a single Atlanta document.

---

**[SLIDE 10 — TEASE]**

The piece covers more than what's in this video.

It names the lawsuit putting four billion dollars of statutory damages on one city's surveillance vendor. It names the federal case that pulled seven point eight million dollars back from a therapy app for selling session data. It names the Supreme Court case that didn't reach the constitutional question.

And it names the one tool that already works. The one method of accountability that surfaced every story in this video.

Three words. The piece tells you what they are.

---

**[SLIDE 11 — END]**

The Civic Node. Subscribe free at drinkyouroj.substack.com.

Weekly. No hype.

---

## Script Notes

**Word count:** ~735
**Estimated runtime:** 5:15 at ~140 wpm (TCN-natural pace)
**Voice register:** 7/10 (Hank-Vox blend) — verified against `workspace/core/anti-ai-writing-style.md`

**Refrain markers (read slow each time):**
- The bypass refrain has three forms across the script. Read each slower than the surrounding text. The Slide 7 closer is the climactic form — give it the longest beat before delivery.
  - Slide 2: "The rule applies to the government. The company holds the database."
  - Slide 5: "The rule still exists on paper. / The function still gets performed."
  - Slide 7 (climax): "In each case, the protection was written to govern one party. The work was contracted to another."

**Breath / pacing cues:**
- Slide 1: hold the silence after "The same audit reported both." for ~1 second before "The pattern shows up in four places."
- Slide 3: brief pause before "That's the floor. The smallest example." — this is the setup for the Slide 4 scale escalation.
- Slide 4: brief pause before "There are roughly forty-five hundred police departments on this network." This is the scale punch.
- Slide 5: full beat between "The rule still exists on paper." and "The function still gets performed." Two complete sentences delivered as the refrain — they need to land separately.
- Slide 6: the anaphora needs space. Half-beat between each pair. Full beat between the explanatory body and the anaphora pairs.
- Slide 7: same anaphora pacing as Slide 6. The closer "In each case, the protection was written to govern one party. The work was contracted to another." gets the longest pause before delivery — this is the climax of the entire piece.
- Slide 8: hold the silence after "Something turned it back on." for ~1 second.
- Slide 8: final line "By whom, the company says, can't be known." — deliver dry. Don't punch it. The deadpan is the joke.
- Slide 9: hold the silence after "The cameras are still on." for ~1 second before the last line.
- Slide 10: pause after "Three words." before "The piece tells you what they are."

**Supersedence (if the article has fact-corrections post-narration):**
- (none on this pass — Script Notes block surfaces corrections when present)

**Cold-open candidate** (for thumbnail / title-skill downstream use):
- "Atlanta has a sanctuary policy. The cameras don't." — the bisected sentence that compresses the contradiction. The article opener pairs the city's policy statement with the search-log numbers from the same audit; the trailer leads with that contradiction because it's the article's strongest "wait, what?" moment.

**Refrain candidate** (for slideshow skill downstream use):
- "The protection was written to govern one party. The work was contracted to another." Three variations are placed across Slides 2, 5, and 7. Slide 7's full-form line is the climax delivery; slideshow design should make that slide breathe (more whitespace, larger type on the closer).

**Cuts from the article** (what we deliberately did not cover, for Tease slide reference):
- Gibbs Mura class action specifics — California Civil Code § 1798.90.55(b), $2,500-per-violation damages floor (Slide 10 names the $4B exposure but not the legal mechanism).
- Murthy v. Missouri specifics — Barrett's June 2024 standing dismissal; the March 2026 settlement naming CISA, the Surgeon General, and the CDC (Slide 10 names the case but not the settlement structure).
- CISA's "switchboarding" program — Director Jen Easterly's framing of the information environment as "cognitive infrastructure"; the House Judiciary Committee report.
- The SCREEN Act — federal bill to expand state-level age-verification regimes nationally.
- The Tea app breach — selfies and driver's licenses ending up on 4chan; address data used to pinpoint home locations.
- Oregon SB 1516 — March 2026 state law restricting Flock data sharing to Oregon agencies and requiring 30-day public audit reports.
- BetterHelp specifics — the FTC enforcement action that produced $7.8M in consumer refunds (Slide 10 names the dollar figure but not the FTC mechanism).
- National security letter / subpoena distinction in detail (Slide 7 names NSLs briefly; the article's fuller treatment of bypass-of-judicial-review is cut).
- "Follow the vendor" — the three-word working instrument named in the article's closing section. This is the answer the Tease slide opens the loop around. (Deliberate echo of "follow the money" — positions the instrument inside an existing investigative tradition.)
- The My Debug section's framing — that the author wrote The Jawboning Papers (CISA / speech beat) and Atlanta Passed a Sanctuary Resolution (immigration beat) before noticing they were the same story from two angles. The "no journalist owns this beat" argument.
- Pierce County, Georgia's four "Border Patrol Assist" searches one month after Flock claimed federal pilots ended.
- Flock CEO Garrett Langley's "inadvertently provided inaccurate information" admission about federal contracts.

**Visual density flags** (for slideshow skill — slides that will need a panel-split):
- (none on this pass — narration re-paced from 9 slides to 11 slides specifically to fit the ≤25 visible-words/slide budget. Slide 6 and Slide 7 each carry one anaphora pair-couplet, ~19 visible words apiece, comfortably under budget. Slide 7's spoken-only closer line — "In each case, the protection was written to govern one party. The work was contracted to another." — is delivered aurally but does not need to appear on the slide; if the slideshow skill chooses to display it, that decision will push Slide 7 over budget and a panel-split or third anaphora slide would be needed.)
