...
The bear case has everything it needs. Google published a compression breakthrough that [compresses AI inference memory 6x](https://research.google/blog/turboquant-redefining-ai-efficiency-with-extreme-compression/). Half of America's planned data centers have been cancelled or delayed. Chinese factories are ramping toward volume production. This is the setup for a price collapse.
...
- Inline source links: 19 (unchanged from v3)
...
- **Opener replaced:**
  - Four-paragraph opener replaced with Variant A (Expectation-Subversion technique) from tcn-opener skill
  - Old opener led with Article 1 callback in sentence 1, burying the hook ("By every standard model... DDR5 should be at $200. It's at $400.") in paragraph 2
  - New opener: opens on the price fact in sentence 1; names the bear case; pivots on "The paper is wrong."; Article 1 reference subordinated to paragraph 4 after the hook is set
  - Opener-close contract: the gap between what every model predicts and what's actually happening; close pays this with "Supply shocks resolve. Architectures don't."
  - Word count reduction: ~42 words (opener from ~176 to ~134 words)

- **Unchanged:**
  - All body sections (Forces 1–5), Pattern section, close
  - All 19 inline source links
  - Draft notes from v3 (corrections, wiki audit) carried forward implicitly; this revision is opener-only
