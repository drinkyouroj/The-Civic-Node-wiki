...
| 7 | Generate the full model-ready prompt following the structure of the existing exemplar prompt files (composition, palette, lighting, negative prompts, aspect ratio, model directives, alt text, caption). |
...
| `ai-image-prompts-skill` (anthropic-skills) | Recommends prompts from a library by category. Different job — this skill writes *bespoke* prompts that follow the flagship DNA. The library skill can be useful when the user wants prompt-structure ideas for a *specific* concept they've already locked in via this skill. Layer, don't replace. |
| `nano-banana-pro-prompts-recommend-skill` | Same — library recommender, layered not replaced. |
...
7. GENERATE THE FULL MODEL-READY PROMPT. Follow the structure of the existing exemplar prompt files (workspace/drafts/the-71-billion-bluff-cover-prompt.md is the cleanest reference). Include: composition description, typography (if any), palette specification, lighting, style references, negative prompts (always include "no embedded text in the image" / "no annotation labels" / "no chart titles" / "no UPC or magazine furniture"), aspect ratio (always 16:9, 1456 × 816), model directives. Also produce alt text and a one-line caption.
